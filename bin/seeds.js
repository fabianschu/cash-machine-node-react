...
db.query("DROP TABLE IF EXISTS projects")
  ...
  .then(() =>
    db.query(
      'CREATE TABLE IF NOT EXISTS "userProfiles" (ID SERIAL PRIMARY KEY, "name" VARCHAR(30), "street" VARCHAR(50), "zip" VARCHAR(30), "city" VARCHAR(30), "email" VARCHAR(30), "phone" VARCHAR(30), "iban" VARCHAR(30), "bic" VARCHAR(30), "taxId" VARCHAR(30), "uId" VARCHAR(30), "userId" INTEGER REFERENCES users("id"))'
    )
  )
  .then(() =>
    db.query(
      'CREATE TABLE IF NOT EXISTS "customers" (ID SERIAL PRIMARY KEY, "firm" VARCHAR(30) UNIQUE, "firstName" VARCHAR(30), "lastName" VARCHAR(30), "street" VARCHAR(30), "zip" VARCHAR(30), "city" VARCHAR(30), "country" VARCHAR(30), "taxId" VARCHAR(30), "hourlyRate" REAL, "active" BOOL DEFAULT true, "userId" INTEGER REFERENCES users("id"))'
    )
  )
  .then(() =>
    db.query(
      'CREATE TABLE IF NOT EXISTS "invoices" (ID SERIAL PRIMARY KEY, "title" VARCHAR(60), "status" VARCHAR(30), "customerId" INTEGER REFERENCES customers("id"), "createdAt" TIMESTAMP, "updatedAt" TIMESTAMP, "userId" INTEGER REFERENCES users("id"))'
    )
  )
  .then(() =>
    db.query(
